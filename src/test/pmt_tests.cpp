...
BOOST_AUTO_TEST_CASE(pmt_test1)
{
    ...
    for (int i = 0; i < 12; i++) {
        ...
        for (int att = 1; att < 15; att++) {
            ...
            std::vector<unsigned int> vApollon;
            uint256 merkleRoot2 = pmt2.ExtractMatches(vMatchTxid2, vApollon);
            ...
            for (int j=0; j<4; j++) {
                ...
                uint256 merkleRoot3 = pmt3.ExtractMatches(vMatchTxid3, vApollon);
...
BOOST_AUTO_TEST_CASE(pmt_malleability)
{
    ...
    std::vector<unsigned int> vApollon;
    BOOST_CHECK(tree.ExtractMatches(vTxid, vApollon).IsNull());
}
