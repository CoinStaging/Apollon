...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total apollon balance of the wallet\n"
...
UniValue createsporkkeypair(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"apollonaddr\" : \"apollonaddr\", (string) The Apollon public address \n"
    ...
    if (isValid) {
        ...
        CTxDestination dest = address.Get();
        //get public address
        ret.push_back(Pair("apollonaddr", currentAddress));
        CScript scriptPubKey = GetScriptForDestination(dest);
        //get scriptpubkey
...
UniValue insync(const UniValue& params, bool fHelp)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", apollonnodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", apollonnodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", apollonnodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", apollonnodeSync.GetBlockchainSynced()));
        objStatus.push_back(Pair("IsApollonnodeListSynced", apollonnodeSync.IsApollonnodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", apollonnodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", apollonnodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", apollonnodeSync.IsFailed()));
        return objStatus;
    }
    ...
    if(strMode == "next")
    {
        ...
        return "sync updated to " + apollonnodeSync.GetAssetName();
    }
...
bool getAddressFromApollon(AddressType const & type, const uint160 &hash, std::string &address)
{
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, AddressType> > &addresses)
{
    if (params[0].isStr()) {
        ...
        if (!address.GetApollonKey(hashBytes, type)) {
...
UniValue getaddressmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "getaddressmempool\n"
                        "\nReturns all mempool deltas for an address (requires addressapollon to be enabled).\n"
                        "\nArguments:\n"
                        "{\n"
                        "  \"addresses\"\n"
                        "    [\n"
                        ...
                        "    \"apollon\"  (number) The related input or output apollon\n"
                        "    \"satoshis\"  (number) The difference of duffs\n"
                        "    \"timestamp\"  (number) The time the transaction entered the mempool (seconds)\n"
                        "    \"prevtxid\"  (string) The previous txid (if spending)\n"
                        "    \"prevout\"  (string) The previous transaction output apollon (if spending)\n"
                        "  }\n"
                        "]\n"
                        "\nExamples:\n"
    ...
    std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> > apollones;

    if (!mempool.getAddressApollon(addresses, apollones)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
    }

    std::sort(apollones.begin(), apollones.end(), timestampSort);

    UniValue result(UniValue::VARR);

    for (std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> >::iterator it = apollones.begin(); it != apollones.end(); it++) {

        std::string address;
        if (!getAddressFromApollon(it->first.type, it->first.addressBytes, address)) {
        ...
        delta.push_back(Pair("apollon", (int)it->first.apollon));
...
UniValue getaddressutxos(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "getaddressutxos\n"
                        "\nReturns all unspent outputs for an address (requires addressapollon to be enabled).\n"
                        "\nArguments:\n"
                        "{\n"
                        "  \"addresses\"\n"
                        "    [\n"
                        ...
                        "    \"outputApollon\"  (number) The output apollon\n"
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++) {
        ...
        std::string address;
        if (!getAddressFromApollon(it->first.type, it->first.hashBytes, address)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unknown address type");
        }

        output.push_back(Pair("address", address));
        output.push_back(Pair("txid", it->first.txhash.GetHex()));
        output.push_back(Pair("outputApollon", (int)it->first.apollon));
...
UniValue getaddressdeltas(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1 || !params[0].isObject())
        throw runtime_error(
                "getaddressdeltas\n"
                        "\nReturns all changes for an address (requires addressapollon to be enabled).\n"
                        "\nArguments:\n"
                        "{\n"
                        "  \"addresses\"\n"
                        "    [\n"
                        ...
                        "    \"txid\"  (string) The related txid\n"
                        "    \"apollon\"  (number) The related input or output apollon\n"
                        "    \"blockapollon\"  (number) The related block apollon\n"
                        "    \"height\"  (number) The block height\n"
                        "    \"address\"  (string) The base58check encoded address\n"
                        "  }\n"
                        "]\n"
    ...
    for (std::vector<std::pair<uint160, AddressType> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (start > 0 && end > 0) {
            if (!GetAddressApollon((*it).first, (*it).second, addressApollon, start, end)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        } else {
            if (!GetAddressApollon((*it).first, (*it).second, addressApollon)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
    }
    ...
    for (std::vector<std::pair<CAddressApollonKey, CAmount> >::const_iterator it=addressApollon.begin(); it!=addressApollon.end(); it++) {
        std::string address;
        if (!getAddressFromApollon(it->first.type, it->first.hashBytes, address)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unknown address type");
        }
        ...
        delta.push_back(Pair("txid", it->first.txhash.GetHex()));
        delta.push_back(Pair("apollon", (int)it->first.apollon));
        delta.push_back(Pair("blockapollon", (int)it->first.txapollon));
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "getaddressbalance\n"
                        "\nReturns the balance for an address(es) (requires addressapollon to be enabled).\n"
                        "\nArguments:\n"
                        "{\n"
                        "  \"addresses\"\n"
                        "    [\n"
    ...
    for (std::vector<std::pair<uint160, AddressType> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (!GetAddressApollon((*it).first, (*it).second, addressApollon)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address. If this is not an address in your wallet, set addressapollon=1 in the conf file.");
        }
    }
...
UniValue getaddresstxids(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "getaddresstxids\n"
                        "\nReturns the txids for an address(es) (requires addressapollon to be enabled).\n"
                        "\nArguments:\n"
                        "{\n"
                        "  \"addresses\"\n"
                        "    [\n"
...
UniValue getspentinfo(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1 || !params[0].isObject())
        throw runtime_error(
                "getspentinfo\n"
                        "\nReturns the txid and apollon where an output is spent.\n"
                        "\nArguments:\n"
                        "{\n"
                        "  \"txid\" (string) The hex string of the txid\n"
                        "  \"apollon\" (number) The start block height\n"
                        "}\n"
                        "\nResult:\n"
                        "{\n"
                        "  \"txid\"  (string) The transaction id\n"
                        "  \"apollon\"  (number) The spending input apollon\n"
                        "  ,...\n"
                        "}\n"
                        "\nExamples:\n"
                + HelpExampleCli("getspentinfo", "'{\"txid\": \"0437cd7f8525ceed2324359c2d0ba26006d92d856a9c20fa0241106ee5a597c9\", \"apollon\": 0}'")
                + HelpExampleRpc("getspentinfo", "{\"txid\": \"0437cd7f8525ceed2324359c2d0ba26006d92d856a9c20fa0241106ee5a597c9\", \"apollon\": 0}")
        );

    UniValue txidValue = find_value(params[0].get_obj(), "txid");
    UniValue apollonValue = find_value(params[0].get_obj(), "apollon");

    if (!txidValue.isStr() || !apollonValue.isNum()) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid txid or apollon");
    }

    uint256 txid = ParseHashV(txidValue, "txid");
    int outputApollon = apollonValue.get_int();

    CSpentApollonKey key(txid, outputApollon);
    CSpentApollonValue value;

    if (!GetSpentApollon(key, value)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get spent info");
    }

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("txid", value.txid.GetHex()));
    obj.push_back(Pair("apollon", (int)value.inputApollon));
...
UniValue gettotalsupply(const UniValue& params, bool fHelp)
{
    ...
    if(!pblocktree->ReadTotalSupply(total))
        throw JSONRPCError(RPC_DATABASE_ERROR, "Cannot read the total supply from the database. This functionality requires -addressapollon to be enabled. Enabling -addressapollon requires reapolloning.");
...
bool getZerocoinSupply(CAmount & amount) {
    using xap_rec = std::pair<CAddressApollonKey, CAmount>;
    std::vector<xap_rec> addressApollon;

    if(!GetAddressApollon(uint160(), AddressType::zerocoinSpend, addressApollon))
        return false;

    auto predicate = [](xap_rec const & lhs, xap_rec const & rhs) {
        if(lhs.first.blockHeight < rhs.first.blockHeight)
            return true;
        if(lhs.first.blockHeight > rhs.first.blockHeight)
            return false;
        return lhs.first.txapollon < rhs.first.txapollon;
    };

    if(!std::is_sorted(addressApollon.begin(), addressApollon.end(), predicate))
        std::sort(addressApollon.begin(), addressApollon.end(), predicate);

    using spend_value = std::pair<uint256, size_t>;
    std::map<CBigNum, spend_value> originalSerials;
    CAmount amt = 0;

    for(xap_rec const & idr : addressApollon) {
        CTransaction tx;
        uint256 hash;
...
UniValue getzerocoinsupply(const UniValue& params, bool fHelp)
{
    ...
    if(!getZerocoinSupply(total))
        throw JSONRPCError(RPC_DATABASE_ERROR, "Cannot read the zerocoin supply from the database. This functionality requires -addressapollon to be enabled. Enabling -addressapollon requires reapolloning.");
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "addressapollon",       "gettotalsupply",         &gettotalsupply,         false },
