...
public:
     ...
     *     1) we have an active Indexnode,
     *     2) we have a valid Indexnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Indexnode address
    bool CheckSignature(const CPubKey& pubKeyIndexnode);
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 XAP - checking if valid indexnode)
...
private:
    ...
    std::vector<CDarkSendEntry> vecEntries; // Indexnode/clients entries
    ...
    const CBlockIndex *pCurrentBlockIndex; // Keep track of current block apollon
    ...
    /// As a client, submit part of a future mixing transaction to a Indexnode to start the process
    ...
    /// Get Indexnode updates about the progress of mixing
...
public:
    CIndexnode* pSubmittedToIndexnode;
