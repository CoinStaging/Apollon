...
// A helper object for signing messages from Apollonnodes
...
public:
     ...
     *     1) we have an active Apollonnode,
     *     2) we have a valid Apollonnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Apollonnode address
    bool CheckSignature(const CPubKey& pubKeyApollonnode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, apollonnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 XAP - checking if valid apollonnode)
...
private:
    ...
    // Keep track of the used Apollonnodes
    std::vector<CTxIn> vecApollonnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Apollonnode/clients entries
    ...
    const CBlockApollon *pCurrentBlockApollon; // Keep track of current block apollon
    ...
    /// As a client, submit part of a future mixing transaction to a Apollonnode to start the process
    ...
    /// Get Apollonnode updates about the progress of mixing
...
public:
    CApollonnode* pSubmittedToApollonnode;
