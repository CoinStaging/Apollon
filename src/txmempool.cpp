...
{
     LOCK(cs);
    for (apolloned_transaction_set::const_iterator mi = mapTx.begin(); mi != mapTx.end(); ++mi){
...
bool CTxMemPool::addUnchecked(const uint256 &hash, const CTxMemPoolEntry &entry, setEntries &setAncestors,
    ...
    apolloned_transaction_set::iterator newit = mapTx.insert(entry).first;
    ...
    if (!entry.GetTx().IsZerocoinSpend() && !entry.GetTx().IsSigmaSpend() && !entry.GetTx().IsZerocoinRemint()) {
        ...
        newit->vTxHashesXap = vTxHashes.size() - 1;
    }
...
void CTxMemPool::removeUnchecked(txiter it) {
    ...
    if (!it->GetTx().IsZerocoinSpend() && !it->GetTx().IsSigmaSpend() && !it->GetTx().IsZerocoinRemint()) {
        ...
        if (vTxHashes.size() > 1) {
            vTxHashes[it->vTxHashesXap] = std::move(vTxHashes.back());
            vTxHashes[it->vTxHashesXap].second->vTxHashesXap = it->vTxHashesXap;
            vTxHashes.pop_back();
...
void CTxMemPool::addAddressApollon(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    LOCK(cs);
    const CTransaction& tx = entry.GetTx();
...
bool CTxMemPool::getAddressApollon(std::vector<std::pair<uint160, AddressType> > &addresses,
...
bool CTxMemPool::removeAddressApollon(const uint256 txhash)
{
    LOCK(cs);
...
void CTxMemPool::addSpentApollon(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    LOCK(cs);

    const CTransaction& tx = entry.GetTx();
    std::vector<CSpentApollonKey> inserted;
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        CSpentApollonKey key = CSpentApollonKey(input.prevout.hash, input.prevout.n);
        CSpentApollonValue value = CSpentApollonValue(txhash, j, -1, prevout.nValue, addressType, addressHash);
...
bool CTxMemPool::getSpentApollon(CSpentApollonKey &key, CSpentApollonValue &value)
{
    LOCK(cs);
    mapSpentApollon::iterator it;
...
bool CTxMemPool::removeSpentApollon(const uint256 txhash)
{
    LOCK(cs);
    mapSpentApollonInserted::iterator it = mapSpentInserted.find(txhash);

    if (it != mapSpentInserted.end()) {
        std::vector<CSpentApollonKey> keys = (*it).second;
        for (std::vector<CSpentApollonKey>::iterator mit = keys.begin(); mit != keys.end(); mit++) {
            mapSpent.erase(*mit);
        }
...
void CTxMemPool::removeForReorg(const CCoinsViewCache *pcoins, unsigned int nMemPoolHeight, int flags) {
    ...
    for (apolloned_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        if (!CheckFinalTx(tx, flags) || !CheckSequenceLocks(*this, tx, flags, &lp, validLP)) {
            ...
            BOOST_FOREACH(const CTxIn &txin, tx.vin) {
                apolloned_transaction_set::const_iterator it2 = mapTx.find(txin.prevout.hash);
                if (it2 != mapTx.end())
                    continue;
...
void CTxMemPool::removeForBlock(const std::vector <CTransaction> &vtx, unsigned int nBlockHeight,
    ...
    try {
        ...
        BOOST_FOREACH(const CTransaction &tx, vtx)
        {
            ...
            apolloned_transaction_set::iterator i = mapTx.find(hash);
            if (i != mapTx.end())
                entries.push_back(*i);
        }
        BOOST_FOREACH(const CTransaction &tx, vtx)
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const {
    ...
    for (auto it = mapNextTx.cbegin(); it != mapNextTx.cend(); it++) {
        ...
        apolloned_transaction_set::const_iterator it2 = mapTx.find(hash);
...
bool CTxMemPool::CompareDepthAndScore(const uint256 &hasha, const uint256 &hashb) {
    LOCK(cs);
    apolloned_transaction_set::const_iterator i = mapTx.find(hasha);
    if (i == mapTx.end()) return false;
    apolloned_transaction_set::const_iterator j = mapTx.find(hashb);
...
namespace {
    ...
    public:
        bool operator()(const CTxMemPool::apolloned_transaction_set::const_iterator &a,
                        const CTxMemPool::apolloned_transaction_set::const_iterator &b) {
...
std::vector <CTxMemPool::apolloned_transaction_set::const_iterator> CTxMemPool::GetSortedDepthAndScore() const {
    std::vector <apolloned_transaction_set::const_iterator> iters;
    AssertLockHeld(cs);

    iters.reserve(mapTx.size());

    for (apolloned_transaction_set::iterator mi = mapTx.begin(); mi != mapTx.end(); ++mi) {
        iters.push_back(mi);
    }
...
std::shared_ptr<const CTransaction> CTxMemPool::get(const uint256 &hash) const {
    LOCK(cs);
    apolloned_transaction_set::const_iterator i = mapTx.find(hash);
    if (i == mapTx.end())
        return nullptr;
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    LOCK(cs);
    // Estimate the overhead of mapTx to be 15 pointers + an allocation, as no exact formula for boost::multi_apollon_contained is implemented.
...
int CTxMemPool::Expire(int64_t time) {
    LOCK(cs);
    apolloned_transaction_set::apollon<entry_time>::type::iterator it = mapTx.get<entry_time>().begin();
    setEntries toremove;
...
void CTxMemPool::TrimToSize(size_t sizelimit, std::vector <uint256> *pvNoSpendsRemaining) {
    ...
    while (!mapTx.empty() && DynamicMemoryUsage() > sizelimit) {
        apolloned_transaction_set::apollon<descendant_score>::type::iterator it = mapTx.get<descendant_score>().begin();
