...
private:
    int block;
    uint256 txid;
    unsigned int xap; // apollon within block
    uint32_t property;
...
public:
    ...
    CMPMetaDEx()
      : block(0), xap(0), property(0), amount_forsale(0), desired_property(0), amount_desired(0),
    ...
    CMPMetaDEx(const std::string& addr, int b, uint32_t c, int64_t nValue, uint32_t cd, int64_t ad,
      ...
      : block(b), txid(tx), xap(i), property(c), amount_forsale(nValue), desired_property(cd), amount_desired(ad),
...
// ---------------
//! Set of objects sorted by block+xap
