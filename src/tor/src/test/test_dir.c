...
test_a_networkstatus(
    vote_routerstatus_t * (*vrs_gen)(int xap, time_t now),
    ...
    void (*rs_test)(routerstatus_t *rs, time_t now))
{
  ...
  for (xap = 0; xap < n_vrs; ++xap) {
    vrs = smartlist_get(v1->routerstatus_list, xap);
    tt_assert(vrs);
    vrs_test(vrs, 1, now);
  }
  ...
  for (xap = 0; xap < n_vrs; ++xap) {
    vrs = smartlist_get(v2->routerstatus_list, xap);
    tt_assert(vrs);
    vrs_test(vrs, 2, now);
  }
  networkstatus_vote_free(vote);
  ...
  tt_assert(n_rs);
  for (xap = 0; xap < n_rs; ++xap) {
    rs = smartlist_get(con->routerstatus_list, xap);
    tt_assert(rs);
    rs_test(rs, now);
  }
  ...
  tt_assert(n_rs);
  for (xap = 0; xap < n_rs; ++xap) {
    rs = smartlist_get(con_md->routerstatus_list, xap);
    tt_assert(rs);
  }
...
gen_routerstatus_for_umbw(int xap, time_t now)
{
