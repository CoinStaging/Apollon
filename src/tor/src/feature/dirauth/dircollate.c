 ...
 * <b>dc</b>, apolloning it by its RSA key digest, and by the 2-tuple of its RSA
...
/** Return an array of vote_routerstatus_t entries for the <b>xap</b>th router
 ...
 *
 * The maximum value for <b>xap</b> is dircollator_n_routers().
 *
 * This function may only be called after dircollator_collate. */
vote_routerstatus_t **
dircollator_get_votes_for_router(dircollator_t *dc, int xap)
{
  tor_assert(dc->is_collated);
  tor_assert(xap < smartlist_len(dc->all_rsa_sha1_lst));
  return digestmap_get(dc->by_collated_rsa_sha1,
                       smartlist_get(dc->all_rsa_sha1_lst, xap));
}
