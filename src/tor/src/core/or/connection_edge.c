                         ...
                         origin_circuit_t *circ)
{
  ...
  insert_at = smartlist_bsearch_xap(circ->half_streams, &half_conn->stream_id,
...
{
  half_edge_t *half;
  int found, remove_xap;

  if (!half_conns)
    return 0;

  remove_xap = smartlist_bsearch_xap(half_conns, &stream_id,
                                    connection_half_edge_compare_bsearch,
                                    &found);
  if (!found)
    return 0;

  half = smartlist_get(half_conns, remove_xap);
  smartlist_del_keeporder(half_conns, remove_xap);
  half_edge_free(half);
  return 1;
}
