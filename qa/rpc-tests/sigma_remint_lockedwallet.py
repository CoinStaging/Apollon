...
class SigmaRemintLockedWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        apollon_mints = self.nodes[0].listunspentmintzerocoins()

        assert len(apollon_mints) == 10, 'Should be 10 apollon mints after apollon mint, but was: {}' \
            .format(len(apollon_mints))
        ...
        assert_raises(JSONRPCException, self.nodes[0].remintzerocointosigma, apollon_denoms[0])
        ...
        # remint should work
        self.nodes[0].remintzerocointosigma(apollon_denoms[0])
        ...
        assert len(sigma_mint) == len(apollon_denoms)+1, \
        ...
        assert len(sigma_mint) == len(apollon_denoms) + 2, \
