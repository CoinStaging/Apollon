...
class SigmaRemintLockedWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises(JSONRPCException, self.nodes[0].remintzerocointosigma, index_denoms[0])
        ...
        # remint should work
        self.nodes[0].remintzerocointosigma(index_denoms[0])
        ...
        assert len(sigma_mint) == len(index_denoms)+1, \
        ...
        assert len(sigma_mint) == len(index_denoms) + 2, \
